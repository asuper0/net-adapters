...
mod from_heim_net {
    ...
    impl Nic {
        pub fn new(
            ...
            gateway: Option<Vec<IpAddr>>,
            dns: Option<Vec<IpAddr>>,
        ) -> Result<Nic> {
            let nic = match dhcp_on {
                true => Nic {
                    index,
                    guid: guid.to_string(),
                ...
                false => {
                    if address.is_none() {
                        return Err(anyhow::Error::msg("address list is empty"));
                    }
        ...
        pub fn dhcp_server(&self) -> &Option<IpAddr> {
            &self.dhcp_server
        }
    ...
    impl PartialEq for Nic {
        fn eq(&self, other: &Self) -> bool {
            self.guid == other.guid
                ...
                && self.dhcp_server == other.dhcp_server
